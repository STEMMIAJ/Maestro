...
# Mapa de segmento de justiça → alias do tribunal
# CNJ: NNNNNNN-DD.AAAA.J.TT.OOOO
# J=8 (estadual), TT=13 (MG) → TJMG
# J=5 (federal), TT=06 (TRF6/MG) → TRF6
TRIBUNAL_MAP = {
    "8.13": "api_publica_tjmg",    # Justiça Estadual MG
    "5.06": "api_publica_trf6",    # Justiça Federal MG (TRF6)
    "5.03": "api_publica_trf3",    # Justiça Federal (TRF3, se houver)
}
...
BASE_DIR = Path(__file__).parent.parent.parent  # ~/Desktop/ANALISADOR FINAL/
...
def cnj_sem_formatacao(cnj):
    """Remove formatação do CNJ: 5001615-98.2025.8.13.0074 → 50016159820258130074"""
    return re.sub(r'[^0-9]', '', cnj)


def detectar_tribunal(cnj):
    """Detecta alias do tribunal pelo CNJ."""
    limpo = cnj_sem_formatacao(cnj)
    if len(limpo) >= 16:
        justica = limpo[13]   # posição 14 (0-indexed 13)
        ramo = limpo[14:16]   # posições 15-16
...
def buscar_processo(cnj, timeout=15):
    """Busca um processo na API DataJud. Retorna dict com dados ou None."""
    alias = detectar_tribunal(cnj)
    if not alias:
        log(f"Tribunal não identificado para {cnj}", "AVISO")
        # Tenta TJMG como fallback
        alias = "api_publica_tjmg"

    url = f"{BASE_URL}/{alias}/_search"
    numero = cnj_sem_formatacao(cnj)
    ...
    try:
        ...
        log(f"Timeout ao buscar {cnj}", "AVISO")
        return None
    except requests.exceptions.HTTPError as e:
        log(f"Erro HTTP ao buscar {cnj}: {e}", "ERRO")
        return None
    except Exception as e:
        log(f"Erro ao buscar {cnj}: {e}", "ERRO")
        return None
